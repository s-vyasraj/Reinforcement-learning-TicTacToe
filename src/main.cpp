...
static
status PlayPerson(TTTPlayer &t1, TTTPlayer &t2, Result_t &result, Player &winner)
{
	Board b(t1.GetChar(), t2.GetChar());
    unsigned int row, col;
    std::string s1, s2;
    ...
    for (int i=0; ; ++i) {
        ...
        s1 = b.GetCurrentBoardState();
        t1.UpdateBoardStatus(s1);
        if (t1.GetPersonMove(row, col) == OK) {
	        b.UpdateMove(row,col, t1.GetPlayerType());
        } else {
            std::cout <<"Fatal error .. user input \n";
            assert(0);
        }
        if (b.IsGameEnd()) break;
...
main()
{
    TTTPlayer human('o',PLAYER_1), t1('o',PLAYER_1), t2('x',PLAYER_2);
    Player winner;
    Result_t result;
...
#define RUN_COUNT 1000    
    ...
    /* Play a real person */
    if (PlayPerson(human, t2, result, winner) == OK) {
        if (result == Win) {
            std::cout << "Game ended Winner: ";
            switch (winner) { 
                case PLAYER_1:
